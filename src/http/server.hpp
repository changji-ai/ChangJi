...
struct Options {
    ...
    /// （Dockerfile 最后一行 `CMD ["changji", "--host", "0.0.0.0",
    /// "--port", "8080"]`），不依赖这个默认值。
    ///
    /// **没有 `serve` 这个子命令。** 这儿原来把那行写成
    /// `changji serve --host …`，照抄下来跑只会得到「不认识的选项：serve」
    /// ——参数循环从 argv[1] 起就只认 `--` 开头的，没有位置参数那一层。
...
/// 让系统给一个没人在用的回环端口。回 0 = 没要到。
///
/// **桌面端起进程内引擎时用它，不写死 8080。** 这台机器上很可能已经有一个
/// 8080 上的引擎在跑（另一条会话、另一部片子），连上去就是往别人的项目里
/// 写东西——而界面上什么都不会说。
///
/// 要到端口之后到 `run()` 真去 bind 之间有一个很小的窗口，别人可能抢走；
/// 抢走的表现是 run() 那头 bind 失败，调用方再要一个就是。
int pick_free_port(const std::string& host = "127.0.0.1");

/// 从**别的线程**请求停服。`run()` 随后返回。
///
/// 命令行那一档靠 Ctrl+C（Crow 自己的信号处理）停；桌面端没有信号可收，
/// 关窗那一下要主动停——而且**必须等 `run()` 返回之后再 `evict_all()`**，
/// 顺序见 main.cpp 里那段（ggml 的设备表和我们握着模型的静态量在不同编译
/// 单元里，反过来会当场 abort）。
///
/// 没在跑就什么都不做。
void request_stop();
