...
///
/// `upscale` 是这幅画面被放大了几倍（1 = 没放大）。装配补水印时要用它——
/// 补的那层得**盖住被一起放大的旧水印**，所以尺寸按"放大前算好再乘回去"，
/// 不是按最终画面算。
///
/// ⚠️ 两者不等，根源是那条 32px 下限：832×480 的镜头水印被顶到 32，放大
/// 2 倍就是 64；而按最终的 960 短边算只有 53——小 17%，旧的从左上角露出来，
/// 看着就是重影（实测过）。短边够大（≥ 580）时两者自然相等。
///
